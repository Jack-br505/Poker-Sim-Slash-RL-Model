#Modify the game class in base sim to add chips, raising, standing, folding, and such
import sys
...
class game():
    ...
    def play_game(self):
        ...
        for player in self.order:
            ...
            #if player is first or second they are the blinds 
            if player == self.order[0]:
                print(f'Player {player} is the small blind and must bet 1 chip')
                bets[player] += 1
                self.chips[player] -= 1

                bets[player], raise_value = self.player_action(player, bets[player], raise_value)

            elif player == self.order[1]:
